...
\]
for some $\beta >1$ that will be discussed later.  For each $t\in \Z_{\ge0}$, we let $w_t=2^{\ell_t}$, so that Kraft's Inequality requires $\sum_{t=0}^\infty w_t^{-1} \le 1$.  We have
\[
   w_0^{-1} = 2^{-\ell_0} = e^{-\beta/c} \approx 1-\beta/c \enspace .
\]
Therefore, in order to satisfy Kraft's Inequality, we must choose codeword lengths that satisfy
\[
   \sum_{t=1}^\infty 2^{-\ell_t} < \beta/c \enspace .
\]
...
\begin{align*}
    ...
    & = n\log c + \sum_{i\in I_0} \ell_0
       ...
       & = n\log c + |I_0|\ell_0
          + \sum_{i\in I_{>0}} t_i(\log c - \ell_0) \\
       & = m\log c + |I_0|\ell_0
           - \sum_{i\in I_{>0}} t_i\ell_0 \\
       & = m\log c + |I_0|\ell_0
           - (m-n)\ell_0 \\
       & \le m\log c + (1-2/c)m\ell_0
           - (m-n)\ell_0 & \text{(justify with Chernoff)}\\
       & \le m\log c - m\ell_0/c 
\end{align*}
...
\[
  \sum_{t=1}^\infty 2^{-\ell_t} = \sum_{t=1}^\infty 2^{-\ell_t} \le 1-e^{-\beta/c} \approx \beta/c \enspace .
\]
...
This estimate is not quite good enough.  Instead, we can use the slightly more elaborate estimate (valid when $t\equiv 0\pmod 4$):
\[
     a_t \le \left((c-2)( (c-1)^3+(c-1)^2 )\right)   ^{t/4}
\]
obtained by counting the number of anagram-free strings of length 4.
