...
\begin{equation}
   ...
   \le \begin{cases}
     0 & \text{if $t_i = 0$} \\
     t_i\log c + (t_i-1)\log(c-1) + (\beta/c)\log e - t_i(\alpha/c)\log e) - \ell_{t_i} 
        & \text{otherwise} \\
     % t_i(\log c - (\alpha/c)\log e) +(\beta/c)\log e - \ell_{t_i} 
        % & \text{otherwise}
   \end{cases}  \eqlabel{xi-i}
\end{equation}
for some $1 > \alpha > \beta > 0$.
...
\begin{align*}
    ...
    & = \log c + (n-1)\log(c-1) + d_0\ell_0 \\ 
      & \qquad {} + \sum_{t=1}^m d_t\left(t\log c + (\beta/c)\log e + (t-1)\log(c-1) - t(\alpha/c)\log e\right) 
      \\
    & = \log c + (n-1)\log(c-1) + d_0(\log(c/(c-1))+(\beta/c)\log e) \\ 
      & \qquad {} + \sum_{t=1}^m d_t\left(t\log c + (\beta/c)\log e + (t-1)\log(c-1) - t(\alpha/c)\log e\right) 
      \\
    & = \log c + (n-1)\log(c-1) + m(\beta/c)\log e + d_0\log(c/(c-1)) \\ 
        & \qquad {} + \sum_{t=1}^m d_t\left(t\log c + (t-1)\log(c-1) - t(\alpha/c)\log e\right) 
      \\
    & = \log c + (n-1)\log(c-1) + m\log c + m(\beta/c)\log e - d_0\log(c-1) \\ 
        & \qquad {} + \sum_{t=1}^m d_t\left((t-1)\log(c-1) - t(\alpha/c)\log e\right) 
      \\
    & = \log c + (n-1)\log(c-1) + m\log c + m(\beta/c)\log e - n\log(c-1) - \sum_{t=1}^m d_t t(\alpha/c)\log e 
        \\
    & = \log (c/(c-1)) + m\log c + m(\beta/c)\log e - \sum_{t=1}^m d_t t(\alpha/c)\log e 
        \\
    & = \log (c/(c-1)) + m\log c + m(\beta/c)\log e - (m-n)(\alpha/c)\log e 
      \\
    & < m\log c - k
\end{align*}
...
\[
   \ell_t = t\log c + (t-1)\log(c-1) + (\beta/c)\log e - t(\alpha/c)\log e - \log a_t   \enspace .
\]
