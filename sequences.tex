...
\begin{proof}
  ...
  Now $s_1',\ldots,s_\ell'$ is a sequence of unique tuples on which we may apply the argument to the preceding paragraph.   Finally, for any $s_{\ell+1},\ldots,s_{2\ell}$ such that $s_1,\ldots,s_\ell$ is a trigger, $s_1',\ldots,s_{2\ell}'= s_1',\ldots,s_{\ell}'\cdot \phi(s_{\ell+1},\ldots,s_{2\ell})$ is also a trigger.
\end{proof}

The preceding lemma works well for $\ell \le |\Sigma|$, but we need something stronger when $\ell > |\Sigma|$.  I hope something like this is true:
